...
AMM::DDSManager <TCPL1> *mgr1 = new AMM::DDSManager<TCPL1>("config/tcp_bridge_ajams.xml",
                                                           "manikin_1");

AMM::DDSManager <TCPL2> *mgr2 = new AMM::DDSManager<TCPL2>("config/tcp_bridge_ajams.xml",
                                                           "manikin_2");
/**
...
public:
    ...
    TCPBridgeListener(AMM::DDSManager <TCPBridgeListener> *tmgr) : mgr{tmgr} { }
...
class TCPL1 : public ListenerInterface {
public:
    std::map <std::string, AMM::EventRecord> eventRecords;
    std::string manikin_id;
    ...
    void onNewCommand(AMM::Command &c, eprosima::fastrtps::SampleInfo_t *info) {
        ...
        if (!c.message().compare(0, sysPrefix.size(), sysPrefix)) {
            ...
            if (value.find("START_SIM") != std::string::npos) {
                ...
                simControl.timestamp(ms);
                simControl.type(AMM::ControlType::RUN);
                mgr1->WriteSimulationControl(simControl);
                std::string tmsg = "ACT=START_SIM;mid=" + manikin_id;
                s->SendToAll(tmsg);
            } else if (value.find("STOP_SIM") != std::string::npos) {
                currentStatus = "NOT RUNNING";
    ...
    void HandleCapabilities(Client *c, std::string const &capabilityVal) {
        ...
        od.description();
        mgr1->WriteOperationalDescription(od);
...
public:
    ...
    void onNewCommand(AMM::Command &c, eprosima::fastrtps::SampleInfo_t *info) {
        ...
        if (!c.message().compare(0, sysPrefix.size(), sysPrefix)) {
            ...
            if (value.find("START_SIM") != std::string::npos) {
                ...
                simControl.timestamp(ms);
                simControl.type(AMM::ControlType::RUN);
                mgr2->WriteSimulationControl(simControl);
                std::string tmsg = "ACT=START_SIM;mid=" + manikin_id;
                s->SendToAll(tmsg);
            } else if (value.find("STOP_SIM") != std::string::npos) {
                currentStatus = "NOT RUNNING";
    ...
    void HandleCapabilities(Client *c, std::string const &capabilityVal) {
        ...
        od.capabilities_schema(capabilityVal);
        od.description();
        mgr2->WriteOperationalDescription(od);
...
public:
    ...
    void InitializeManager1() {
        LOG_INFO << "Initializing manikin 1 manager and listener";

        mgr1->InitializeCommand();
        mgr1->InitializeInstrumentData();
        mgr1->InitializeSimulationControl();
        mgr1->InitializePhysiologyModification();
        mgr1->InitializeRenderModification();
        mgr1->InitializeAssessment();
        mgr1->InitializePhysiologyValue();
        mgr1->InitializePhysiologyWaveform();
        mgr1->InitializeEventRecord();
        mgr1->InitializeOperationalDescription();
        mgr1->InitializeModuleConfiguration();
        mgr1->InitializeStatus();
        mgr1->InitializeOmittedEvent();

        mgr1->CreateOperationalDescriptionPublisher();
        mgr1->CreateModuleConfigurationPublisher();
        mgr1->CreateStatusPublisher();
        mgr1->CreateEventRecordPublisher();
        mgr1->CreatePhysiologyValueSubscriber(&t1, TCPL1::onNewPhysiologyValue);
        mgr1->CreatePhysiologyWaveformSubscriber(&t1, TCPL1::onNewPhysiologyWaveform);
        mgr1->CreateCommandSubscriber(&t1, TCPL1::onNewCommand);
        mgr1->CreateSimulationControlSubscriber(&t1, *TCPL1::onNewSimulationControl);
        mgr1->CreateAssessmentSubscriber(&t1, *TCPL1::onNewAssessment);
        mgr1->CreateRenderModificationSubscriber(&t1, *TCPL1::onNewRenderModification);
        mgr1->CreatePhysiologyModificationSubscriber(&t1, *TCPL1::onNewPhysiologyModification);
        mgr1->CreateEventRecordSubscriber(&t1, *TCPL1::onNewEventRecord);
        mgr1->CreateOmittedEventSubscriber(&t1, *TCPL1::onNewOmittedEvent);
        mgr1->CreateOperationalDescriptionSubscriber(&t1, *TCPL1::onNewOperationalDescription);
        mgr1->CreateModuleConfigurationSubscriber(&t1, *TCPL1::onNewModuleConfiguration);
        mgr1->CreateRenderModificationPublisher();
        mgr1->CreatePhysiologyModificationPublisher();
        mgr1->CreateSimulationControlPublisher();
        mgr1->CreateCommandPublisher();
        mgr1->CreateInstrumentDataPublisher();
        mgr1->CreateAssessmentPublisher();
        m_uuid.id(mgr1->GenerateUuidString());
    }

    void InitializeManager2() {
        LOG_INFO << "Initializing manikin 2 manager and listener";
        mgr2->InitializeCommand();
        mgr2->InitializeInstrumentData();
        mgr2->InitializeSimulationControl();
        mgr2->InitializePhysiologyModification();
        mgr2->InitializeRenderModification();
        mgr2->InitializeAssessment();
        mgr2->InitializePhysiologyValue();
        mgr2->InitializePhysiologyWaveform();
        mgr2->InitializeEventRecord();
        mgr2->InitializeOperationalDescription();
        mgr2->InitializeModuleConfiguration();
        mgr2->InitializeStatus();
        mgr2->InitializeOmittedEvent();

        mgr2->CreateOperationalDescriptionPublisher();
        mgr2->CreateModuleConfigurationPublisher();
        mgr2->CreateStatusPublisher();
        mgr2->CreateEventRecordPublisher();
        mgr2->CreatePhysiologyValueSubscriber(&t2, *TCPL2::onNewPhysiologyValue);
        mgr2->CreatePhysiologyWaveformSubscriber(&t2, *TCPL2::onNewPhysiologyWaveform);
        mgr2->CreateCommandSubscriber(&t2, *TCPL2::onNewCommand);
        mgr2->CreateSimulationControlSubscriber(&t2, *TCPL2::onNewSimulationControl);
        mgr2->CreateAssessmentSubscriber(&t2, *TCPL2::onNewAssessment);
        mgr2->CreateRenderModificationSubscriber(&t2, *TCPL2::onNewRenderModification);
        mgr2->CreatePhysiologyModificationSubscriber(&t2, *TCPL2::onNewPhysiologyModification);
        mgr2->CreateEventRecordSubscriber(&t2, *TCPL2::onNewEventRecord);
        mgr2->CreateOmittedEventSubscriber(&t2, *TCPL2::onNewOmittedEvent);
        mgr2->CreateOperationalDescriptionSubscriber(&t2, *TCPL2::onNewOperationalDescription);
        mgr2->CreateModuleConfigurationSubscriber(&t2, *TCPL2::onNewModuleConfiguration);
        mgr2->CreateRenderModificationPublisher();
        mgr2->CreatePhysiologyModificationPublisher();
        mgr2->CreateSimulationControlPublisher();
        mgr2->CreateCommandPublisher();
        mgr2->CreateInstrumentDataPublisher();
        mgr2->CreateAssessmentPublisher();
        m_uuid.id(mgr2->GenerateUuidString());
    }
...
void *Server::HandleClient(void *args) {
    ...
    std::string uuid = mgr1->GenerateUuidString();
